...
# imputed_data <- mice(d3, m = 5, method = 'pmm', maxit = 50, seed = 500)
...
# Combine the dataframes with an additional column to identify the source
d3$Source <- "d3"
...
# Predict on the external validation dataset
...
# Histogram of predicted probabilities using ggplot2
hist_plot <- ggplot(ex_val_lr, aes(x=predicted_prob)) + geom_histogram(binwidth=0.01)

hist_plot


# Combine the calibration plot, histogram and ROC curve
p <- grid.arrange(calibration_plot, hist_plot, nrow = 1)

p

# Convert actual_outcome to numeric
...
actual_outcome <- ex_val_lr$covid_ed
...
# This is the first way I coded the calibration plot
cal_obj <- lrm(covid_ed ~ predicted_prob, data = ex_val_lr, x = TRUE, y = TRUE)
cal_plot <- calibrate(cal_obj, method = "boot", B = 100)
calibration_plot <- ggplotify::as.ggplot(function() plot(cal_plot))
...
predicted_class <- ifelse(probabilities >= as.numeric(optimal_threshold), 1, 0)

# Check the result
table(predicted_class)
length(predicted_class)
length(d3_clean$covid_ed)



# Convert actual outcome to numeric
actual_outcome <- as.numeric(as.character(d3_clean$covid_ed))
...
pdf("calibration_plots.pdf")

# Fit the logistic regression model on the internal validation dataset
model <- glm(covid_ed ~ AGE_IN_YEARS + RESPIRATORY_RATE + SPO2, data = in_val_lr, family = binomial)


# Predict on the internal validation dataset
in_val_lr$predicted_prob <- predict(model, newdata = in_val_lr, type = "response")

# Predict on the external validation dataset
ex_val_lr$predicted_prob <- predict(model, newdata = ex_val_lr, type = "response")

# Fit BBQ model on the predicted probabilities of the internal validation dataset
bbq_model <- rms::val.prob(in_val_lr$predicted_prob, in_val_lr$covid_ed)

# Create bins for the predicted probabilities
breaks <- quantile(in_val_lr$predicted_prob, probs = seq(0, 1, length.out = 101))
in_val_lr$prob_bin <- cut(in_val_lr$predicted_prob, breaks = breaks, labels = FALSE, include.lowest = TRUE)
ex_val_lr$prob_bin <- cut(ex_val_lr$predicted_prob, breaks = breaks, labels = FALSE, include.lowest = TRUE)

bin_means <- tapply(in_val_lr$covid_ed, in_val_lr$prob_bin, mean)

ex_val_lr$calibrated_prob <- bin_means[as.character(ex_val_lr$prob_bin)]

# Brier Score after calibration
ex_val_lr$covid_ed <- as.numeric(as.character(ex_val_lr$covid_ed))
brier_score <- mean((ex_val_lr$covid_ed - ex_val_lr$calibrated_prob)^2)
print(paste("Brier Score after calibration: ", brier_score))

# Set up a multi-panel plot
par(mfrow = c(2, 2))

# Histogram of predicted probabilities pre-calibration
hist(ex_val_lr$predicted_prob, main = "Histogram of Predicted Probabilities\n(Pre-Calibration)", xlab = "Predicted Probability", col = "lightblue", breaks = 50, xlim = c(0, 1))

# Calibration plot pre-calibration
cal_obj_pre <- rms::lrm(covid_ed ~ predicted_prob, data = ex_val_lr, x = TRUE, y = TRUE)
cal_plot_pre <- rms::calibrate(cal_obj_pre, method = "boot", B = 100)
plot(cal_plot_pre, xlab = "Predicted Probability", ylab = "Observed Proportion", main = "Pre-Calibration Plot")

# Histogram of predicted probabilities post-calibration
hist(ex_val_lr$calibrated_prob, main = "Histogram of Predicted Probabilities\n(Post-Calibration)", xlab = "Calibrated Probability", col = "lightblue", breaks = 50, xlim = c(0, 1))

# Calibration plot post-calibration
ex_val_lr_post <- ex_val_lr
ex_val_lr_post$predicted_prob <- ex_val_lr$calibrated_prob
cal_obj_post <- rms::lrm(covid_ed ~ predicted_prob, data = ex_val_lr_post, x = TRUE, y = TRUE)
cal_plot_post <- rms::calibrate(cal_obj_post, method = "boot", B = 100)
plot(cal_plot_post, xlab = "Calibrated Probability", ylab = "Observed Proportion", main = "Post-Calibration Plot")
